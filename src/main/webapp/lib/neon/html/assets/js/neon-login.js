		...
		neonLogin.$container.validate({
			...
			submitHandler: function(ev)
			{
				...
				password = hex_md5(password.trim());
		  		  $.post("./au?time="+(new Date().getTime()),{username : username,password: password}, function(result){
							...
							setTimeout(function()
									{
										neonLogin.setPercentage(100, function()
										{
											...
											setTimeout("window.location.href = './systemuser/functionlist'", 200);
//											setTimeout("window.location.href = './welcome'", 200);
										}, 1);
										
									}, 120);
				  		}else{
				  			bootbox.alert("用户名密码不正确,请输入正确的账户信息!");
				  		}
				  });
						...
						neonLogin.setPercentage(random_pct, function()
						{
							...
							setTimeout(function()
							{
								neonLogin.setPercentage(100, function()
								{
									...
									// Redirect to the page
									setTimeout("window.location.href = './map.jsp'", 600);
								}, 2);
